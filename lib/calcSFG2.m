function Fltr = calcSFG2(lddr) % used to generate transfer function
...
%
  ...
  TF2 = {};
  TF2{1} = [s*Coeffs(1,2)/(s + Coeffs(1,5)), ...
  s*(Coeffs(1,4) - Coeffs(1,3)*Coeffs(1,5))/(s + Coeffs(1,5)), 0;...
  Coeffs(1,2)/(s + Coeffs(1,5)), ...
  (Coeffs(1,4) + s*Coeffs(1,3))/(s + Coeffs(1,5)), 0];
  TF2{1}.u = {'vin', 'x3_2', 'null'};
  TF2{1}.y = {'x1_1', 'x3_1'};

  Coeffs(N-1, :) = calcSFG_Coeffs(elems(NmbElems-2), elems(NmbElems-1), elems(NmbElems), 0, 1/lddr.R2);
  Coeffs(N, :) = calcSFG_Coeffs(elems(NmbElems - 1), elems(NmbElems), elem0, 0, 1/lddr.R2);

  b(N) = Coeffs(N,1)/(1 - Coeffs(N-1,3)*Coeffs(N,1));
  c(N) = Coeffs(N,2)*(1 + b(N)*Coeffs(N-1,3));
  f(N) = Coeffs(N,5)*(1 + b(N)*Coeffs(N-1,3));
  TF2{N} = [s*c(N)/(s + f(N)), ...
  s*(Coeffs(N,4) - Coeffs(N,3)*Coeffs(N,5))/(s + f(N)), ...
  s*b(N)/(s + f(N)); ...
  c(N)/(s + f(N)), ...
  (Coeffs(N,4) + s*Coeffs(N,3))/(s + f(N)) ...
  b(N)/(s + f(N))];
  TF2{N}.u = {x3Cells{N-1}, 'null', x1Cells{N-1}};
  ...
  for i = 2:(N - 1)
    ...
    b(i) = Coeffs(i,1)/(1 - Coeffs(i-1,3)*Coeffs(i,1));
    c(i) = Coeffs(i,2)*(1 + b(i)*Coeffs(i-1,3));
    f(i) = Coeffs(i,5)*(1 + b(i)*Coeffs(i-1,3));
    TF2{i} = [s*c(i)/(s + f(i)), ...
    s*(Coeffs(i,4) - Coeffs(i,3)*Coeffs(i,5))/(s + f(i)), ...
    s*b(i)/(s + f(i)); ...
    c(i)/(s + f(i)), ...
    (Coeffs(i,4) + s*Coeffs(i,3))/(s + f(i)) ...
    b(i)/(s + f(i))];
    TF2{i}.u = {x3Cells{i-1}, x3Cells{i+1}, x1Cells{i-1}};
