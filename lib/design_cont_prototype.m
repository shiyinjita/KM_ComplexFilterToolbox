function [H, E, F, P, e_] = design_cont_prototype(p,px,ni,wp,ws,as,ap,type)
% [H, E, F, P, e_] = design_cont_prototype(p,px,ni,wp,ws,as,ap,type) designs continuous-time prototype
...
[p2, px2, wp2, ws2, as2] = predistortSpecs(p, px, wp, ws, as);
...
A = 20*log_rsps(H,j*2*tan(pi*[wsb1 wsb2]));
...
% Hd = zpk(zd,pd,kd,1); % Make zero, pole system
warning on all
