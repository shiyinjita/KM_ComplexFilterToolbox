...
[X1, elem1, elem2, elem3] = rmv2XPoles(Zin, P(2), P(3), lddr);
[X2, elem4, elem5, elem6] = rmv2XPoles(X1, P(1), P(4), lddr);
[X3, elem7] = rmvSCmplx(X2, lddr);
lddr.R2 = X3.K;
% lddr.makeSingleTerm();
dispLddr(lddr);

% plot_lddr(H, lddr, 2*pi*wp*sclFctr, 2*pi*ws*sclFctr, 'b', [-25 25 -40 1]);
...
% [ax1 ax2, f, ym] = plotRspns(xout, wp_, 'b', [-40, 2]);

tic
out = simLddrMC(lddr2, xin, wp_, 0, 2e-4, 25, [-40, 2]);
toc

drawnow;
cscdHndl = gcf;
% print('../examples/Figures/Lddr_1_4_0_MC','-dpdf');
...
drawnow;
cscdHndl = gcf;
% print('../examples/Figures/1_4_0_MC','-dpdf');
print('../examples/Figures/Cscd_1_4_0_MC','-dpng');
