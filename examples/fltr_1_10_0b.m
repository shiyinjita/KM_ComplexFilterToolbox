% This filter has ten moveable poles. It's stop band specification is asymmetric
% The specifications are given symmetric to dc and then the specifications are
% right shifted by 2 rad.
%
...
p = [-10 -7 -5 -3 -2 -1.6 1.6 2 3 5]; % initial guess at finite loss poles
ni=1; % number of loss poles at infinity
wp = []; ws = [];
wp(1) = -1; % lower passband edge
wp(2) = 1; % upper passband edge
ws = [-15 -2.1 -1.3 1.2 15];
as = [40 80 80 40 40];
...
% plot_crsps(H,wp,ws,'b',[-20 20 -120 2]);
...
[X1, elem1, elem2, elem3] = rmv2XPoles(z11, P(5), P(6), lddr);
[X2, elem4, elem5, elem6] = rmv2XPoles(X1, P(4), P(7), lddr);
[X3, elem7, elem8, elem9] = rmv2XPoles(X2, P(3), P(8), lddr);
[X4, elem10, elem11, elem12] = rmv2XPoles(X3, P(2), P(9), lddr);
[X5, elem13, elem14, elem15] = rmv2XPoles(X4, P(1), P(10), lddr);
%[X5, elem13, elem14] = rmvUsingS2(X4, P(9), lddr);
[X6, elem16] = rmvSCmplx(X5, lddr);
%lddr.R2 = 0.7378;
%lddr.R2 = 1;
lddr.makeSingleTerm();
